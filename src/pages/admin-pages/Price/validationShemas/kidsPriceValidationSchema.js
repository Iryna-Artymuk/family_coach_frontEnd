...
export const kidsPriceValidationSchema = Yup.object().shape({
  ...
  singleConsultation: Yup.object().shape({
    type: Yup.string()
      .min(2)
      .max(20)
      .required('ERROR:Введіть назвупакету')
      .matches(
        /[а-яА-ЯЁёЇїІіЄєҐґ0-9.,;:!?-]+/g,
        'Введіть назву літерами кирилиці'
      ),
    duration: Yup.string()
      ...
      .required('ERROR:Введіть тривалість консультації'),
    price: Yup.number()
      .required('ERROR:Введіть вартість консультації')
      .test(
        'Is positive?',
        'ERROR: ціна повинна бути числом яке більше 0!',
        value => value >= 0
